...
def rasterizeRoads(roads, landcover, road_speed_map):
    ...
    filtered = filter(lambda f: f['properties']['highway'] in road_speed_map,
                      roads)

    speedsurface = features.rasterize(
        ((f['geometry'],
          road_speed_map[f['properties']['highway']]) for f in filtered),
...
def rasterizeAllRoads(roads, landcover, road_speed_map, maxspeed=True):
    ...
    road_types = set([feature['properties']['highway'] for feature in road_features])
